...
class BERTopicAnalyzer:
    def __init__(self, topic_number, topn):
    ...
    def generate_keyword_combinations(self, top_keywords):
        combinations = list(itertools.combinations(top_keywords, 2))[:5]
        return [' '.join(combination) for combination in combinations]

analyzer = BERTopicAnalyzer(topic_number=0, topn=5)
analyzer.analyze_topics(df)
